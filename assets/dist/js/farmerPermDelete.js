async function confirmFarmerDeletion(farmerId) {
...
async function deleteFarmer(farmerId) {
  try {
    const response = await fetch("farmerPermDelete.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
      body: new URLSearchParams({ farmer_id: farmerId }),
    });
...
async function confirmDeleteAllFarmer() {
  const swalWithBootstrapButtons = Swal.mixin({
    customClass: {
      confirmButton: "btn btn-success",
      cancelButton: "btn btn-danger",
...
async function checkProfileExists(farmerId) {
  try {
    const response = await fetch("farmerPermDelete.php", {
      method: "POST",
      headers: {
