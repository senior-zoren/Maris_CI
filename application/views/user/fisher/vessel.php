...
<head>
  ...
  <title>Fishing Vessels | MARIS</title>
...
<body class="hold-transition sidebar-mini sidebar-collapse">
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column nav-flat nav-child-indent" data-widget="treeview" role="menu"
            ...
            <li class="nav-item">
              <a href="<?= base_url('agriculturist/vessel') ?>" class="nav-link active">
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Registered Fishing Vessels</h1>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      </div>
          ...
          <table class="table table-striped" id="vesselTable">
            ...
            <tbody style="text-align: center; vertical-align: middle;">
              <?php if (!empty($vesselData)): ?>
                <?php foreach ($vesselData as $row): ?>
                  <tr>
                    <td><?= htmlspecialchars($row['reg_num']); ?></td>
                    <td><?= htmlspecialchars($row['f_name'] . ' ' . $row['l_name'] . ' ' . $row['suffix']); ?></td>
                    <td><?= htmlspecialchars($row['year_built']); ?></td>
                    <td><?= htmlspecialchars($row['equipments']); ?></td>
                    <td><?= htmlspecialchars($row['material_used']); ?></td>
                    <td><?= htmlspecialchars($row['gross_tonnage']); ?></td>
                    <td><?= htmlspecialchars($row['net_tonnage']); ?></td>
                    <td><?= htmlspecialchars($row['engine_make']); ?></td>
                    <td><?= htmlspecialchars($row['vessel_name']); ?></td>
  ...
  <script>
    $(function () {
      ...
      }).buttons().container().appendTo('#vesselTable_wrapper .col-md-6:eq(0)');
    });
  </script>
</body>
